...
<div id="k2ModuleBox<?php echo $module->id; ?>" class="k2ItemsBlock <?php echo $params->get('moduleclass_sfx'); ?>">
  ...
  <ul class="clearfix">
    ...
    <li class="itemContent <?php echo ($key%2) ? "odd" : "even"; if(count($items)==$key+1) echo ' lastItem'; elseif($key==0) echo ' firstItem'; else echo ' centerItem' ; ?>">
      ...
      <?php
				...
				if (preg_match ($regex, $item->introtext, $matches)) {
					$image = $matches[0];
					$item->introtext = str_replace ($image, '', $item->introtext);
				}
			?>
	    <?php if ($image) : ?>
      ...
      <div class="moduleItemExtraFields">
	      <ul>
	        <?php 
					...
					if($extraField->value):
					?>
					<li class="type<?php echo ucfirst($extraField->type); ?> group<?php echo $extraField->group; ?> <?php echo 'row-'.$cls ?>">
