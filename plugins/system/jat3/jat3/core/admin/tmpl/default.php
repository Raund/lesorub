...
<script type="text/javascript">
	...
	window.addEvent('load', function (e){
		...
		var table_params = $(document.adminForm).getChildren()[1].getElement('table.admintable');
		...
		for(var j=0; j<length; j++){
			if( $(params_extra[0]).getText().indexOf(<?php echo $db->quote(str_replace ('\\', '\\\\', JText::_('There are no Parameters for this item')))?>)<0){
		...
		$(document.adminForm).getChildren()[1].setText('');
		tabswrapmain.injectInside($(document.adminForm).getChildren()[1]);
		...
		if($('additional_information')!=null){
			$('additional_information').injectInside($(document.adminForm).getChildren()[0]);
			$('additional_information').show();
		}

		$(document.adminForm).getChildren()[1].show();
		$(document.adminForm).getChildren()[0].show();
