...
class T3Template extends ObjectExtendable {
	...
	function getLayoutXML () {
		return $this->_theme_info['layout'];
	}
	...
	function genBlockBegin ($block, $name='') {
		...
		?>
		<div id="ja-<?php echo $name ?>" class="wrap<?php echo $clearfix ?>">		
		<?php endif ?>
	...
	function head () {
		...
		$themes[] = array('template', 'default');
		$themes[] = array('engine', 'default');
		$htmls['scrips'] = '';
		$css_urls = array();
		if ($this->isRTL()) {
			foreach ($themes as $theme) {
				$htmls[$theme[0].'.'.$theme[1].'-rtl'] = '';
				$css_urls[$theme[0].'.'.$theme[1].'-rtl'] = array();
			}
		}
		foreach ($themes as $theme) {
			$htmls[$theme[0].'.'.$theme[1].'-browser'] = '';
			$css_urls[$theme[0].'.'.$theme[1].'-browser'] = array();
		}
		foreach ($themes as $theme) {
			$htmls[$theme[0].'.'.$theme[1]] = '';
		...
		if ($this->_js) {
			foreach ($this->_js as $path) {
				if (($url = T3Path::getUrl ($path))) {
					$htmls['scrips'] .= "<script type=\"text/javascript\" src=\"$url\" ></script>\n";
				}
			}
		}
		
		//css
		if ($this->_css && count ($this->_css)) {
			foreach ($this->_css as $media=>$css) {
				foreach ($css as $path) {
					if (($urls = T3Path::get ($path, true))) {
						foreach ($urls as $theme=>$url) {
							//$htmls[$theme] .= "<link type=\"text/css\" rel=\"stylesheet\" href=\"$url\" media=\"$media\" />\n";
							$css_urls [$theme][] = $url;
						}
					}
				}
			...
			foreach ($this->_css as $media=>$css) {
				foreach ($css as $path) {
					$path1 = str_replace('.css', "-$bname.css", $path);
					$path2 = str_replace('.css', "-$bname$bver.css", $path);
					if (($urls = T3Path::get ($path1, true))) {
						foreach ($urls as $theme=>$url) {
							//$htmls[$theme.'-browser'] .= "<link type=\"text/css\" rel=\"stylesheet\" href=\"$url\" media=\"$media\" />\n";
							$css_urls [$theme.'-browser'][] = $url;
						}
					}
					if (($urls = T3Path::get ($path2, true))) {
						foreach ($urls as $theme=>$url) {
							//$htmls[$theme.'-browser'] .= "<link type=\"text/css\" rel=\"stylesheet\" href=\"$url\" media=\"$media\" />\n";
							$css_urls [$theme.'-browser'][] = $url;
						}
			...
			if ($this->isRTL()) {
				foreach ($this->_css as $media=>$css) {
					foreach ($css as $path) {
						$path1 = str_replace('.css', '-rtl.css', $path);
						$path2 = str_replace('.css', "-$bname-rtl.css", $path);
						$path3 = str_replace('.css', "-$bname$bver-rtl.css", $path);
						if (($urls = T3Path::get ($path1, true))) {
							foreach ($urls as $theme=>$url) {
								//$htmls[$theme.'-rtl'] .= "<link type=\"text/css\" rel=\"stylesheet\" href=\"$url\" media=\"$media\" />\n";
								$css_urls [$theme.'-rtl'][] = $url;
							}
						}
						if (($urls = T3Path::get ($path2, true))) {
							foreach ($urls as $theme=>$url) {
		...
		//$html = '';
		//foreach ($htmls as $h) $html = $h.$html;
		
		$html = $htmls['scrips'];
		//create 1 css file to load all css
		$html .= "\n".$this->gencss ($css_urls);
		echo $html;
	}
	
	function gencss ($css_urls) {
		$content = '';
		$devmode = T3Parameter::get('devmode', 0);
		$files = '';
		...
		foreach ($css_urls as $urls) {
			...
			foreach ($urls as $url) {
				...
				if ($devmode == 'yes') {
					//import css
					$content_ .= "@import url(\"{$url[1]}\");\n";
				} else {
					//join css files into one file
					$content_ .= "/* FILE: {$url[1]} */\n".T3Common::compresscss(JFile::read ($url[0]), $url[1])."\n\n";
				}
			}
			$content = $content_.$content;
		}
		$file = md5 ($devmode.$files).'.css';
		$url = T3Cache::store_file ($content, $file);
		if ($url) return "<link type=\"text/css\" rel=\"stylesheet\" href=\"$url\" />\n";
	...
	function getBodyClass () {
		...
		$layout = $this->getLayoutXML();
		if ($bd_cls = T3Common::node_attributes ($layout, 'body-class')) $cls = $cls." ".$bd_cls;
		
		//get custom class
		$custom_cls = T3Hook::_('custom_body_class');
		if ($custom_cls) $cls = $cls." ".$custom_cls;
