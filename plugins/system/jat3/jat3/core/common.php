...
class T3Common {
	...
	function arr_find_child ($element, $name, $attr='', $value='') {
		...
		for ($j=0,$m=count($element['children']);$j<$m;$j++) {
			...
			$child['index'] = $j;
			if (!$attr) return $child; //return the first match by name if not specify attribute
			if (isset($child['attributes'][$attr]) && $child['attributes'][$attr] == $value) {
				return $child;
			}
		}
		return null;
	...
	function get_layout_info ($layout = '') {
		...
		if (!isset($layout_infos[$layout])) {
			...
			} else if ($default_layout_path != $active_layout_path && is_file ($default_layout_path)) {
				$layout_info = $layout_info? T3Common::merge_info (T3Common::getXML($default_layout_path), $layout_info):T3Common::getXML($default_layout_path);
			} else if ($engine_layout_path != $default_layout_path && $engine_layout_path != $active_layout_path && is_file ($engine_layout_path)) {
	...
	function node_children ($node, $name=null, $index = -1) {
		...
		if ($index > -1) return isset ($children[$index])?$children[$index]:null;
		return $children;
	}
