...
class linkBrowserMenu
{
	...
	function getItems($args)
	{
		...
		default:
			...
			break;
		case 'menu':
			$menus = linkBrowserMenu::_menu(1, $args->id);
	...
	function _alias($id)
	{
		...
		$query = 'SELECT id, title, link, alias'
		. ' FROM #__menu'
		. ' WHERE published = 1'
	...
	function _children($id)
	{
		...
		. ' AND access <= 1'//.(int) $user->get('aid')
		;

		$db->setQuery($query, 0);
		return $db->loadResult();
	}
	function _menu($parent = 1, $type = 0)
	{
		$db		=& JFactory::getDBO();
		$user	=& JFactory::getUser();
		...
		$query = 'SELECT m.id, m.title, m.link, m.type, m.alias'
		. ' FROM #__menu AS m'
		. $where
		. ' m.published = 1'
		. ' AND m.parent_id = 1'//.(int) $parent
		//. ' AND m.access <= 1'
		. ' ORDER BY m.title'
		;
		$db->setQuery($query, 0);
