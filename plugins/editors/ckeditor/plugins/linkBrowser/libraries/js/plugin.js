...
var Plugin = new Class({
	...
	getOptions : function(){
		return {
			site 	: document.location.protocol+'//'+document.location.hostname+document.location.pathname, //'http://localhost/joomla_test/administrator',//tinyMCEPopup.editor.documentBaseURI.getURI(true),
			lang 	: 'en',
			params 	: {},
			alerts 	: null
		}
	...
	*/
	...
	var json = JSON.encode({'fn': fn, 'args': args});
	var jsonRequest = new Request.JSON({url: u, onSuccess: function(responseJSON, responseText){
		if(responseJSON.error){
			alert(responseJSON.error);
		}
		r = responseJSON.result || {error: false};
		if(cb){
			cb.pass(r, this)();
		}else{
			return r;
...
//Plugin.implement(new Options, new Events );
Plugin.implement(new Events);
Plugin.implement(new Options);
