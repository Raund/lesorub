...
var Tree = new Class({
	...
	nodeEvents : function(parent){
		...
		//$$('div.tree-row', parent).each(function(el){
		$(parent).getElements('div.tree-row').each(function(el){
		//$(parent).getElements('li.sortableItem').each(function(el){
		...
		$(parent).getElements('div.tree-image').each(function(el){
			var p = this.findParent(el);
			el.addEvent('click', function(e){
				this.toggleNode(e, p);
			}.bind(this))

			if(/*this.getNode(p)*/ this.getNode(p).length > 0){
				el.addClass('open');
			}
		}.bind(this));
		...
		$(this.container).getElements('span').each(function(el){
			...
			if(this.getNode(el.getParent()).length > 0){
				el.addClass('open');
			}
		}.bind(this));
	...
	*/
	getNode : function(parent){
		...
		return $(parent).getElements('ul.tree-node');
		//return $$('ul.tree-node', parent);
	...
	*/
	resetNodes : function(){
		...
		$(this.container).getElements('div.tree-image').each(function(el){
			el.removeClass('open');
		});
	},
	/**
	...
	*/
	findNode : function(id, parent){
		...
		return $(parent.id+' li[id='+ this._escape(encodeURI(id)) +']') || false;
	},
	/**
	...
	*/
	toggleLoader : function(node){
		...
		var span = $(node).getElements('span'), cls = this.options.loaderClass;
		span = span[0];
	...
	*/
	collapseNodes : function(ex){
		...
		$(parent).getElements('li').each(function(el){
	...
	*/
	toggleNodeState : function(node, state){
		var span 	= $(node).getElements('span');
		span = span[0];

		//var span 	= $E('span', node);
		//var div 	= $E('div.tree-image', node);
		var div 	= $(node).getElements('div.tree-image');
		div = div[0];
	...
	*/
	toggleNode : function(e, node){
		...
		//var obj = $$(node,'div.tree-image').hasClass('open');
			if($(node,'div.tree-image').hasClass('open')){
		//	if($E('div.tree-1image', node).hasClass('open')){
